...
* Exposure: FI (FIq:  & FIr: ) 
* outcome: gout (confirmed cases)
* Follow-up period: NHS: 1984-2010  HPFS:1986-2012
* covariates: Analyses were stratified by age (in month), calendar year, and cohort. 
Our multivariable models were adjusted for race (White, Asian, African American or other), 
physical activity (<3, 3 to < 9, 9 to < 18, 18 to < 27, ≥27 mets/wk), 
smoking status (never, former, or current [1-14, 15-24, or ≥25 cigarettes/d]), 
menopausal status (premenopausal or postmenopausal [never, or postmenopausal hormone use], NHS only), 
baseline hypertension (yes or no), baseline high cholesterol (yes or no), total energy intake (quintiles), 
baseline BMI (<21, 21-24.9, 25-29.9, 30-34.9, or>=35). 
history of kidney failure (HPFS only), and diuretic use.
...
%model_phreg1(dsn=pool,event=case, time=tgt, exp=&FIq_,  cov1=,     label=age,food=total,cat=);
%model_phreg1(dsn=pool,event=case, time=tgt, exp=&FIq_,  cov1=&cov, label=mvt,food=total,cat=);
...
set 
pool_case_age_total_       pool_case_mvt_total_
...
ODS RTF FILE='./output/table2_pool_gout.rtf' startpage=no; 
title '/*********************************Main**************************************/';
