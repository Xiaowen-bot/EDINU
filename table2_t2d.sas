...
* Exposure: FI (FIq:  & FIr: )
* outcome: T2D
* Follow-up period: NHS: 1984-2018.6  HPFS:1986-2018.1
...
%include '/udd/n2xwa/urate_Wang/cohort/nhscohort_t2d.sas';
%include '/udd/n2xwa/urate_Wang/cohort/hpfscohort_t2d.sas';
...
proc freq data=pool;tables db2case;run;
...
%let cov = hbpbase cholbase  pmh_ever oc_ever mvit aspirin dbfh  &race_  &actc_   &smkc_  &qtei_  &bmib_;
...
%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=&FIq_,  cov1=,     label=age,food=total,cat=);
%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=&FIq_,  cov1=&cov, label=mvt,food=total,cat=);

%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=&FIqr_,  cov1=,     label=age,food=rank,cat=);
%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=&FIqr_,  cov1=&cov, label=mvt,food=rank,cat=);

/* HR (95% CI) per SD*/
%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=FI_SD,  cov1=,     label=age,food=total,cat=sd);
%model_phreg1(dsn=pool,event=db2case, time=tdb2, exp=FI_SD,  cov1=&cov, label=mvt,food=total,cat=sd);
...
set 
pool_db2case_age_total_       pool_db2case_mvt_total_
pool_db2case_age_rank_        pool_db2case_mvt_rank_
;
...
ODS RTF FILE='./output/table2_pool_t2d.rtf' startpage=no; 
title '/*********************************Main**************************************/';
