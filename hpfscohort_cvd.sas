...
Outcome:            CVD (CHD, cabg, stroke)                                   
Exposure:           Build dietary index: &FIq_                                                                                                                                                                                        
Programmer:         Xiaowen Wang   

Program Name:       /udd/n2xwa/urate_Wang/cohort/hpfscohort_cvd.sas
...
- censor incident CVD, death


**************************************************
* HPFS cohort data (CVD)                *
**************************************************;
/******* General options *************************************/
...
if 0<dtdxnfstr<9999 then dtdxstr=dtdxnfstr;
        ...
        if s1 in (1,2) then hems=1; else hems=0;
        if s1 in (3,4,5) then iscs=1; else iscs=0;
...
dtdxchdcabg=.;
if 0<dtdxchd<9999 then dtdxchdcabg=dtdxchd;
else if 0<dtdxcabg<9999 then dtdxchdcabg=dtdxcabg;
...
proc sort data=GOUTPRS; by id; run;
    ...
    data base0;
    merge diabetes cvd hp_der(in=mst) hp_der_2 h86_cat h90_cat h94_cat h98_cat h02_cat h06_cat h10_cat h14_cat
...
proc datasets nolist;
   delete hp_der hp_der_2 hp86 hp88 hp90 hp92 hp94 hp96 hp98 hp00 hp02 hp04 hp06 hp08 hp10 hp12 hp14 hp16 hp18 dead  gout
          ...
          h86_dt h90_dt h94_dt h98_dt  hmet8616 ahei GOUTPRS
          ;
   run;
...
if actcon>7.5 then active=1; else active=0;
   ...
   %exclude(age86 eq .);   
   %exclude(0 lt dtdxcvdca le irt{i});       
   ...
   %exclude(irt[i-1] lt dtdxcvdca le irt[i]);  *CVD as outcome;
...
data hpfs;
 ...
 cutoff  dtdth  dtdxdb2 db2case tdb2 dtdxchdcabg CHDCA tchdca 
 ...
 alldead dead_can dead_cvd talld prs_urate
 FIq   FIqr
 &FIq_ &FIqr_
 FIcon FIconr
